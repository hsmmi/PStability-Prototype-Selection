...
class LSSm(BaseAlgorithm):
    ...
    def _set_distance_nearest_enemy(self):
        ...
        self.nearest_enemy = np.full(self.X.shape[0], -1)
        self.distance_nearest_enemy = np.full(self.X.shape[0], np.inf)

        for idx in range(self.X.shape[0]):
            for idx2 in range(self.X.shape[0]):
                if (
                    self.y[idx] != self.y[idx2]
    ...
    def select(self) -> np.ndarray:
        ...
        self.local_set = [set() for _ in range(self.X.shape[0])]
        self._set_local_set()

        u = np.zeros(self.X.shape[0])
        h = np.zeros(self.X.shape[0])
